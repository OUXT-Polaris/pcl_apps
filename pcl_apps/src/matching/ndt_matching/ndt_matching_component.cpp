...
: Node("ndt_matching", options)
{
  ...
  auto reference_cloud_callback =
    ...
    boost::shared_ptr<pcl::PointCloud<pcl::PointXYZ>> reference_cloud_(new pcl::PointCloud<pcl::PointXYZ>);
    ...
    if (use_min_max_filter_) {
      ...
      for (const auto & p : reference_cloud_->points) {
        ...
        if (scan_min_range_ < r && r < scan_max_range_) {tmp_ptr->points.push_back(p);}
      }
